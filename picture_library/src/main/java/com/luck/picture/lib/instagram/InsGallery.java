...
//        // 自定义相册右侧文本内容设置
    ...
    public static void openGalleryForSingleImage(Activity activity, ImageEngine engine, CacheResourcesEngine cacheResourcesEngine, OnResultCallbackListener listener) {
        openGalleryForSingleImage(activity, engine, cacheResourcesEngine, null, listener);
    }

    public static void openGalleryForSingleImage(Activity activity, ImageEngine engine, CacheResourcesEngine cacheResourcesEngine, List<LocalMedia> selectionMedia, OnResultCallbackListener listener) {
        applyInstagramOptionsForSingleImage(activity.getApplicationContext(), PictureSelector.create(activity)
    ...
    public static PictureSelectionModel applyInstagramOptionsForSingleImage(Context context, PictureSelectionModel selectionModel) {
        return applyInstagramOptionsForSingleImage(context, InstagramSelectionConfig.createConfig().setCurrentTheme(currentTheme), selectionModel);
    }

    public static PictureSelectionModel applyInstagramOptionsForSingleImage(Context context, InstagramSelectionConfig instagramConfig, PictureSelectionModel selectionModel) {
        return selectionModel
                .setInstagramConfig(instagramConfig)
                .setPictureStyle(createInstagramStyle(context))// 动态自定义相册主题
                .setPictureCropStyle(createInstagramCropStyle(context))// 动态自定义裁剪主题
    ...
    public static void openGalleryForPost(Activity activity, ImageEngine engine, CacheResourcesEngine cacheResourcesEngine, OnResultCallbackListener listener) {
        openGalleryForPost(activity, engine, cacheResourcesEngine, null, listener);
    }

    public static void openGalleryForPost(Activity activity, ImageEngine engine, CacheResourcesEngine cacheResourcesEngine, List<LocalMedia> selectionMedia, OnResultCallbackListener listener) {
        applyInstagramOptionsForPost(activity.getApplicationContext(), PictureSelector.create(activity)
                .openGallery(PictureMimeType.ofAll()))// 全部.PictureMimeType.ofAll()、图片.ofImage()、视频.ofVideo()、音频.ofAudio()
                .imageEngine(engine)// 外部传入图片加载引擎，必传项
                .loadCacheResourcesCallback(cacheResourcesEngine)// 获取图片资源缓存，主要是解决华为10部分机型在拷贝文件过多时会出现卡的问题，这里可以判断只在会出现一直转圈问题机型上使用
                .selectionData(selectionMedia)// 是否传入已选图片
    ...
    public static PictureSelectionModel applyInstagramOptionsForPost(Context context, PictureSelectionModel selectionModel) {
        return applyInstagramOptionsForPost(context, InstagramSelectionConfig.createConfig().setCurrentTheme(currentTheme), selectionModel);
    }

    public static PictureSelectionModel applyInstagramOptionsForPost(Context context, InstagramSelectionConfig instagramConfig, PictureSelectionModel selectionModel) {
        return selectionModel
                .setInstagramConfig(instagramConfig)
                .setPictureStyle(createInstagramStyle(context))// 动态自定义相册主题
                .setPictureCropStyle(createInstagramCropStyle(context))// 动态自定义裁剪主题
